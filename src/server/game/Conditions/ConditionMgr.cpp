...
ConditionList ConditionMgr::GetConditionsForSmartEvent(int32 entryOrGuid, uint32 eventId, uint32 sourceType)
{
	...
	if (itr != m_SmartEventConditions.end())
	{
		...
		{
			cond = (*i).second;
			sLog->outDebug(LOG_FILTER_CONDITIONSYS, "GetConditionsForSmartEvent: found conditions for Event entryOrGuid %u sourceType %u eventId %u", entryOrGuid, sourceType, eventId);
		}
	}
	return cond;
}
